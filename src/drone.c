...
// Computes the repulsive force exerted by a border based on the given parameters.
// The formula used is detailed in the documentation. The parameters can be adjusted
// in the configuration file.
float compute_repulsive_force(float distance, float function_scale,
                      float area_of_effect, float vel_x, float vel_y) {
    // Calculates the repulsive force based on distance, velocity, and configured scaling.
...
int main(int argc, char *argv[]) {
    ...
    drone_current_position.x = prev_x = prev2_x = get_param("drone", "init_pos_x");
    drone_current_position.y = prev_y = prev2_y = get_param("drone", "init_pos_y");
    ...
    // Flag indicating if the program should terminate after receiving a STOP request
    bool to_exit = false;
    ...
    while (1) {
        ...
        for (int i = 0; i <= fd; i++) {
            if (FD_ISSET(i, &reader)) {
                ...
                if (ret == 0) {
                    // If a pipe is closed, log a warning and remove it from monitoring  
                ...
                switch (received[0]) {
                    case 'T':  
                        // If "TH" is received, it indicates a target has been hit  
                        if (received[1] == 'H') {
                            int target_index = 0;
                            float target_x = 0, target_y = 0;

                            // Extract the hit target's index and position  
                            sscanf(received + 2, "|%d|%f,%f", &target_index, &target_x, &target_y);
                            ...
                            if (targets_arr[target_index].x != target_x ||
                                ...
                                printf("%f %f %f %f\n",
                                       ...
                                       target_y);
                                fflush(stdout);
                                logging("ERROR", "Mismatched target and array in drone");
                            } else {
                                // Remove the target from the array  
                                remove_target(target_index, targets_arr, targets_num);
                    ...
                    case 'O':  
                        ...
                        // sprintf(server_message, "Total obstacles updated: %d", obstacles_num);
                        // logging("INFO", server_message);
                    ...
                    default:  
                        // If none of the above, assume the message contains force components  
                        sscanf(received, "%f|%f", &drone_force.x_component, &drone_force.y_component);
                        break;
                }
            }
        }
        ...
        for (int i = 0; i < obstacles_num; i++) {
            ...
            // Apply repulsive force only if within effect range and not too close
            if (distance < obst_of_effect && distance > 1) {
                // Compute the magnitude of the repulsive force
                double force_magnitude =
                    -compute_repulsive_force(distance, 10000, obst_of_effect,
                ...
                total_obstacles_forces.x_component += cos(angle) * force_magnitude;
                total_obstacles_forces.y_component += sin(angle) * force_magnitude;

                // Cap the force to prevent extreme values
                total_obstacles_forces.x_component =
                    fmax(fmin(total_obstacles_forces.x_component, MAX_OBST_FORCES), -MAX_OBST_FORCES);
                total_obstacles_forces.y_component =
                    fmax(fmin(total_obstacles_forces.y_component, MAX_OBST_FORCES), -MAX_OBST_FORCES);
            }
        }
        ...
        for (int i = 0; i < targets_num; i++) {
            ...
            if (distance < targ_of_effect) {
                ...
                // Apply force in the computed direction
                total_targets_forces.x_component += cos(angle) * force_magnitude;
                total_targets_forces.y_component += sin(angle) * force_magnitude;

                // Cap forces to prevent extreme values
                total_targets_forces.x_component =
                    fmax(fmin(total_targets_forces.x_component, MAX_TARG_FORCES), -MAX_TARG_FORCES);
                total_targets_forces.y_component =
                    fmax(fmin(total_targets_forces.y_component, MAX_TARG_FORCES), -MAX_TARG_FORCES);
            }
        }

        // Compute repulsive force from simulation boundaries
        // The function effect is applied only when within 'area_of_effect' from a boundary
        if (prev_x < area_of_effect) {
            walls.x_component = compute_repulsive_force(prev_x, function_scale, area_of_effect,
                                                drone_current_velocity.x_component,
                                                drone_current_velocity.y_component);
        } else if (prev_x > SIMULATION_WIDTH - area_of_effect) {
            walls.x_component = -compute_repulsive_force(SIMULATION_WIDTH - prev_x, function_scale, area_of_effect,
                                                 drone_current_velocity.x_component,
                                                 drone_current_velocity.y_component);
        } else {
            walls.x_component = 0; // No force applied when sufficiently far from boundaries
        }

        // Compute repulsive force from top and bottom boundaries
        if (prev_y < area_of_effect) {
            walls.y_component = compute_repulsive_force(prev_y, function_scale, area_of_effect,
                                                drone_current_velocity.x_component,
                                                drone_current_velocity.y_component);
        } else if (prev_y > SIMULATION_HEIGHT - area_of_effect) {
            walls.y_component = -compute_repulsive_force(SIMULATION_HEIGHT - prev_y, function_scale, area_of_effect,
                                                  drone_current_velocity.x_component,
                                                  drone_current_velocity.y_component);
        } else {
            walls.y_component = 0;
        ...
        // Calculate the new position of the drone using the given physics formula.
        // The same calculation is applied to both x and y axes.

        // Prevent small floating-point values from preventing the velocity from reaching zero.
        // A threshold is applied only when no external forces are acting on the drone.
        ...
        // Enforce simulation boundaries to prevent the drone from escaping the defined area.
        // This also ensures that the repulsive force calculations remain valid.
        // If the force is set too high, the next position might completely bypass the border effect.
        // Special handling is applied near position 0 to avoid infinite force due to mathematical asymptotes.
        ...
        // Compute the current velocity using the change in position over the time step.
        // This calculation ensures that the displayed velocity is as up-to-date as possible.
        // There is a slight delay of one iteration in detecting velocity zero, 
        // but this does not significantly affect the simulation.
        drone_current_velocity.x_component = (drone_current_position.x - prev_x) / T;
        drone_current_velocity.y_component = (drone_current_position.y - prev_y) / T;
        ...
        sprintf(server_message, "%f,%f|%f,%f", 
                drone_current_position.x, drone_current_position.y, 
                drone_current_velocity.x_component, drone_current_velocity.y_component);
