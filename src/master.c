...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NUM_PROCESSES; i++) {
        ...
        if (!child_pids[i]) {
            ...
            char *exec_args[] = {process_names[i], NULL, NULL, NULL, NULL, NULL,
                                NULL,        NULL, NULL, NULL, NULL, NULL};
            char *konsole_arg_list[] = {"konsole", "-e", process_names[i], NULL,
                                        NULL,      NULL, NULL};

            switch (i) {
                case 0:
                    // **Server Process Setup**       
                    // Convert pipe file descriptors to strings for argument passing.
                ...
                case 2:
                    ...
                    Close(map_server[0]); Close(map_server[1]);
                    Close(server_map[0]); Close(server_map[1]);
                    Close(target_server[0]); Close(target_server[1]);
                    Close(server_target[0]); Close(server_target[1]);
                    Close(obstacle_server[0]); Close(obstacle_server[1]);
                    Close(server_obstacle[0]); Close(server_obstacle[1]);
                ...
                case 4:
                    ...
                    Close(server_target[1]);  // Target only reads from the server
                ...
                case 5:
                    ...
                    Close(obstacle_server[0]);  // Obstacle only writes to the server
                    Close(server_obstacle[1]);  // Obstacle only reads from the server

                    // Spawn the obstacle process
                    spawn(exec_args);
                    break;
             }
            //  Spawn the last process: Watchdog (WD), which monitors all other processes
            ...
            switch (i) {
                    case 1: // **Drone has spawned**
                        Close(server_drone[0]); Close(server_drone[1]);
                        Close(drone_server[0]); Close(drone_server[1]);
                        break;

                    case 2: // **Input has spawned**
                        Close(server_input[0]); Close(server_input[1]);
                        Close(input_server[0]); Close(input_server[1]);
                        break;
