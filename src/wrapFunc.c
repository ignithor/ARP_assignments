...
int Shm_open(const char *name, int flag, mode_t mode) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing shm_open: %s, pid: %d, from: %s, line: %d, "
                "awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
int Ftruncate(int fd, __off_t length) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing ftruncate: %s, pid: %d, from: %s, line: "
                "%d, awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
void *Mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset) {
    ...
    if (ret == MAP_FAILED) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing mmap: %s, pid: %d, from: %s, line: %d, "
                "awaiting "
                "termination "
                "from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
...
sem_t *Sem_open(const char *name, int oflag, mode_t mode, unsigned int value) {
    ...
    if (ret == SEM_FAILED) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing sem_open: %s, pid: %d, from: %s, line: %d, "
                "awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
int Sem_init(sem_t *sem, int pshared, int value) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing sem_init: %s, pid: %d, from: %s, line: %d, "
                "awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
int Sem_wait(sem_t *sem) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing sem_wait: %s, pid: %d, from: %s, line: %d, "
                "awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
int Sem_post(sem_t *sem) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing sem_post: %s, pid: %d, from: %s, line: %d, "
                "awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
int Sem_close(sem_t *sem) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing sem_close: %s, pid: %d, from: %s, line: "
                "%d, awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
int Sem_unlink(const char *name) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing sem_unlink: %s, pid: %d, from: %s, line: "
                "%d, awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
void Shm_unlink(const char *name) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing shm_unlink: %s, pid: %d, from: %s, line: "
                "%d, awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
...
void Munmap(void *addr, size_t len) {
    ...
    if (ret < 0) {
        char msg[MAX_STR_LEN];
        sprintf(msg,
                "Error on executing munmap: %s, pid: %d, from: %s, line: %d, "
                "awaiting "
                "termination from WD",
                strerror(errno), getpid(), __FILE__, __LINE__);
        printf("%s\n", msg);
